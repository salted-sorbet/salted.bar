...
Item {
  ...
  // Extra gap between the bar and its screen edge (used by KeyboardPanel
  // to position popups correctly when the bar floats away from the edge).
  property real barMargin: 0
  // Extra margin on the parallel axis (left/right for a horizontal bar)
  // so popups align with the island's horizontal offset.
  property real barParallelMargin: 0
  ...
  component BarPanel: PanelWindow {
    ...
    // --- Floating island tuning ---------------------------------------
    // islandThickness halves the bar's visual thickness (height for a
    // horizontal bar, width for a vertical one) while keeping it centered
    // in the space the layer-shell surface reserves, so surrounding
    // windows still get a correctly sized exclusion zone.
    readonly property real islandThickness: root.barSize
    // Fraction of the screen's long axis the island actually occupies.
    // 0.5 means the bar is half as long as before and centered, with the
    // freed space split evenly as a margin on each side.
    readonly property real islandLengthFraction: 0.5
    readonly property real screenLongAxis: root.vertical
      ? (screen ? screen.height : 0)
      : (screen ? screen.width : 0)
    // The margin required on EACH side to leave islandLengthFraction of
    // the screen's long axis for the bar itself.
    readonly property real islandLengthMargin: Math.round(
      screenLongAxis * (1 - islandLengthFraction) / 2
    )
    readonly property real islandRadius: Math.min(islandThickness / 2, Style.cornerRadius * 2)
    // Gap between the island and the single screen edge it's docked to
    // (top for a top bar, left for a left bar, etc.) so it floats clear
    // of that edge instead of sitting flush against it.
    readonly property real islandEdgeGap: Style.space(10)

    margins {
      top: (root.barHidden && root.position === "top" ? -root.barSize : 0)
        + (root.vertical ? islandLengthMargin : 0)
        + (root.position === "top" ? islandEdgeGap : 0)
      bottom: (root.barHidden && root.position === "bottom" ? -root.barSize : 0)
        + (root.vertical ? islandLengthMargin : 0)
        + (root.position === "bottom" ? islandEdgeGap : 0)
      left: (root.barHidden && root.position === "left" ? -root.barSize : 0)
        + (!root.vertical ? islandLengthMargin : 0)
        + (root.position === "left" ? islandEdgeGap : 0)
      right: (root.barHidden && root.position === "right" ? -root.barSize : 0)
        + (!root.vertical ? islandLengthMargin : 0)
        + (root.position === "right" ? islandEdgeGap : 0)
    }
    ...
    // The window surface itself stays fully transparent now; the visible,
    // rounded "island" is the inner Rectangle below, inset and centered
    // within this surface so it floats clear of the screen edge and (for
    // a horizontal bar) clear of the left/right edges too.
    color: "transparent"
    ...
    Component.onCompleted: {
      root.barMargin = islandEdgeGap
      root.barParallelMargin = islandLengthMargin
    }

    Item {
      id: islandFrame
      anchors.fill: parent
      // Vertical bars keep their full implicit width but shrink to half
      // thickness centered top-to-bottom isn't meaningful the same way, so
      // islandThickness only shortens the cross-axis for horizontal bars.
      anchors.topMargin: !root.vertical ? (parent.height - islandThickness) / 2 : 0
      anchors.bottomMargin: !root.vertical ? (parent.height - islandThickness) / 2 : 0
      ...
      Rectangle {
        ...
        layer.enabled: !root.transparent
